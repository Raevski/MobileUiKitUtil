...
import svg_to_vector_drawable_converter.VdParser.Companion.parsePath
import svg_to_vector_drawable_converter.VdPath.Node.Companion.NodeListToString
import svg_to_vector_drawable_converter.VdPath.Node.Companion.transform
 ...
 */
internal class SvgLeafNode(svgTree: SvgTree?, node: Node?, nodeName: String?) : SvgNode(
    svgTree!!, node!!, nodeName!!
) {
    ...
    private fun convertRGBToHex(svgValue: String): String? {
        ...
        val numbers = functionValue.split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        if (numbers.size != 3) {
            return null
        }
    ...
    override fun transform(a: Float, b: Float, c: Float, d: Float, e: Float, f: Float) {
        ...
        val n = parsePath(mPathData!!)
        if (!(a == 1f && d == 1f && b == 0f && c == 0f && e == 0f && f == 0f)) {
            transform(a, b, c, d, e, f, n)
        }
        mPathData = NodeListToString(n)
    }
    ...
    companion object {
        private val logger = Logger.getLogger(SvgLeafNode::class.java.simpleName)
