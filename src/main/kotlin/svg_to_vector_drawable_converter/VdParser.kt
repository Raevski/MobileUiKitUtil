...
internal class VdParser {
    ...
    var mParseSize: ElemParser = object : ElemParser {
        override fun parse(tree: VdTree, attributes: Attributes) {
            parseSize(tree, attributes)
        }
    }
    var mParsePath: ElemParser = object : ElemParser {
        override fun parse(tree: VdTree, attributes: Attributes) {
            val p = parsePathAttributes(attributes)
            tree.add(p)
        }
    }
    var mParseGroup: ElemParser = object : ElemParser {
        override fun parse(tree: VdTree, attributes: Attributes) {
            val g = parseGroupAttributes(attributes)
            tree.add(g)
        }
    }
    var tagSwitch = HashMap<String, ElemParser>()
    ...
    fun parse(`is`: InputStream?, vdErrorLog: StringBuilder): VdTree? {
        return try {
            val tree = VdTree()
            ...
            xr.contentHandler = object : ContentHandler {
                ...
                @Throws(SAXException::class)
                override fun startElement(s: String, s2: String, s3: String, attributes: Attributes) {
                    if (tagSwitch.containsKey(s3)) {
                        tagSwitch[s3]!!.parse(tree, attributes)
                    }
                    space += " "
                }
    ...
    private fun parseSize(vdTree: VdTree, attributes: Attributes) {
    ...
    fun setNameValue(vgPath: VdPath, name: String, value: String) {
        if (PATH_DESCRIPTION == name) {
            ...
            if (!java.lang.Float.isNaN(vgPath.mStrokeOpacity)) {
                ...
                vgPath.mStrokeColor = vgPath.mStrokeColor or ((0xFF * vgPath.mStrokeOpacity).toInt() shl 24)
            }
    ...
    companion object {
        private val logger = Logger.getLogger(VdParser::class.java.simpleName)
        ...
        ////////////////////////////////////////////////////////////////
        private fun addNode(list: ArrayList<VdPath.Node>, cmd: Char, `val`: FloatArray) {
