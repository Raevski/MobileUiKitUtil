...
object AssetUtil {
    ...
    fun blurredImage(source: BufferedImage, radius: Double): BufferedImage {
        ...
        val sqrtPiSigma22 = Math.sqrt(Math.PI * sigma22)
        ...
        while (y <= r) {
            ...
            while (x <= r) {
                ...
                if (distance2 > radius2) {
                    ...
                    kernelData[index] = (Math.exp(-distance2 / sigma22) / sqrtPiSigma22).toFloat()
                }
    ...
    fun drawEffects(
        ...
        for (effect in fillEffects) {
            ...
            fillOpacity = Math.max(0.0, Math.min(1.0, effect.opacity))
        }
        // Inner shadows
        for (effect in shadowEffects) {
            if (!effect.inner) {
    ...
    fun drawCenterInside(g: Graphics2D, source: BufferedImage, dstRect: Rectangle) {
        ...
        if (srcWidth * 1.0 / srcHeight > dstRect.width * 1.0 / dstRect.height) {
            val scaledWidth = Math.max(1, dstRect.width)
            val scaledHeight = Math.max(1, dstRect.width * srcHeight / srcWidth)
            ...
            val scaledWidth = Math.max(1, dstRect.height * srcWidth / srcHeight)
            val scaledHeight = Math.max(1, dstRect.height)
            val scaledImage: Image = scaledImage(source, scaledWidth, scaledHeight)
            g.drawImage(
                scaledImage,
