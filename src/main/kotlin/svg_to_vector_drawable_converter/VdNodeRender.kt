    ...
    private fun addCommand(
        ...
        while (k < `val`.size) {
            ...
            when (cmd) {
                ...
                'q' -> {
                    path.quadTo(
                        (cx + `val`[k + 0]).toDouble(), (cy + `val`[k + 1]).toDouble(), (cx + `val`[k + 2]).toDouble(),
                        (
                                cy + `val`[k + 3]).toDouble()
                    )
                    cpx = cx + `val`[k + 0]
    ...
    private fun arcToBezier(
        ...
        for (i in 0 until numSegments) {
            ...
            * (Math.sqrt(4 + 3 * tanDiff2 * tanDiff2) - 1) / 3
            ...
            p.curveTo(
                q1x.toFloat().toDouble(), q1y.toFloat().toDouble(), q2x.toFloat().toDouble(), q2y.toFloat().toDouble(),
                e2x.toFloat().toDouble(), e2y.toFloat().toDouble()
            )
            eta1 = eta2
            e1x = e2x
            e1y = e2y
