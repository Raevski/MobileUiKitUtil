...
object Svg2Vector {
    ...
    const val SVG_TRANSFORM = "transform"
    ...
    private val unsupportedSvgNodes = Sets.newHashSet( // Animation elements
        "animate",
        "animateColor",
        "animateMotion",
        "animateTransform",
    ...
    private fun parse(f: File): SvgTree {
        val svgTree = SvgTree()
        val doc = svgTree.parse(f)
        val nSvgNode: NodeList
        // Parse svg elements
        ...
        val root = SvgGroupNode(svgTree, rootNode, "root")
        svgTree.root = root
    ...
    private fun traverseSVGAndExtract(svgTree: SvgTree, currentGroup: SvgGroupNode, item: Node) {
        ...
        for (i in 0 until allChildren.length) {
            ...
            if (SVG_PATH == nodeName || SVG_RECT == nodeName || SVG_CIRCLE == nodeName || SVG_POLYGON == nodeName || SVG_LINE == nodeName) {
                val child = SvgLeafNode(svgTree, currentNode, nodeName + i)
                extractAllItemsAs(svgTree, child, currentNode)
                currentGroup.addChild(child)
            } else if (SVG_GROUP == nodeName) {
                val childGroup = SvgGroupNode(svgTree, currentNode, "child$i")
    ...
    private fun parseDimension(avg: SvgTree, nNode: Node) {
        ...
        for (i in 0 until len) {
            ...
            if (SVG_WIDTH == name) {
                ...
                val strbox = value.split(" ".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
                for (j in avg.viewBox!!.indices) {
    ...
    private fun extractAllItemsAs(avg: SvgTree, child: SvgLeafNode, currentItem: Node) {
        ...
        if (SVG_POLYGON == currentItem.nodeName) {
    ...
    private fun extractRectItem(avg: SvgTree?, child: SvgLeafNode, currentGroupNode: Node) {
        ...
        if (currentGroupNode.nodeType == Node.ELEMENT_NODE) {
            ...
            if (!pureTransparent && avg != null && !java.lang.Float.isNaN(x) && !java.lang.Float.isNaN(y)
    ...
    private fun extractCircleItem(avg: SvgTree?, child: SvgLeafNode, currentGroupNode: Node) {
        ...
        if (currentGroupNode.nodeType == Node.ELEMENT_NODE) {
            ...
            if (!pureTransparent && avg != null && !java.lang.Float.isNaN(cx) && !java.lang.Float.isNaN(cy)) {
    ...
    private fun extractLineItem(avg: SvgTree?, child: SvgLeafNode, currentGroupNode: Node) {
        ...
        if (currentGroupNode.nodeType == Node.ELEMENT_NODE) {
            ...
            if (!pureTransparent && avg != null && !java.lang.Float.isNaN(x1) && !java.lang.Float.isNaN(y1)
    ...
    private fun getSizeString(w: Float, h: Float, scaleFactor: Float): String {
        return """        android:width="${(w * scaleFactor).toInt()}dp"
...
"""
    ...
    private fun writeFile(outStream: OutputStream, svgTree: SvgTree) {
        ...
        fw.write(head)
        val finalWidth = svgTree.w
        val finalHeight = svgTree.h
        fw.write(getSizeString(finalWidth, finalHeight, svgTree.mScaleFactor))
        fw.write(
            """        android:viewportWidth="${svgTree.w}"
"""
        )
        fw.write(
            """        android:viewportHeight="${svgTree.h}">
"""
        )
        svgTree.normalize()
