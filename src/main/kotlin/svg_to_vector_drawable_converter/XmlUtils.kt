...
/** XML Utilities  */
object XmlUtils {
    const val XML_COMMENT_BEGIN = "<!--" //$NON-NLS-1$
    const val XML_COMMENT_END = "-->" //$NON-NLS-1$
    const val XML_PROLOG = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n" //$NON-NLS-1$

    /**
     * Separator for xml namespace and localname
     */
    const val NS_SEPARATOR = ':' //$NON-NLS-1$
    ...
    fun lookupNamespacePrefix(node: Node, nsUri: String): String {
        val defaultPrefix =
            if (SdkConstants.ANDROID_URI == nsUri) SdkConstants.ANDROID_NS_NAME else SdkConstants.APP_PREFIX
    ...
    fun lookupNamespacePrefix(
        ...
        if (SdkConstants.XMLNS_URI == nsUri) {
        ...
        while (node != null && node.nodeType == Node.ELEMENT_NODE) {
            ...
            for (n in attrs.length - 1 downTo 0) {
                ...
                if (SdkConstants.XMLNS == attr.prefix) {
        ...
        if (doc != null) {
            ...
            if (node != null && create) {
                ...
                val attr = doc.createAttributeNS(SdkConstants.XMLNS_URI, SdkConstants.XMLNS_PREFIX + prefix)
                attr.value = nsUri
    ...
    fun fromXmlAttributeValue(escapedAttrValue: String): String {
        var workingString = escapedAttrValue.replace(SdkConstants.QUOT_ENTITY, "\"")
    ...
    fun getUtfReader(file: File): Reader {
        ...
        when (bytes[0]) {
            ...
            0xFF.toByte() -> {
                if (length >= 2
                    ...
                    return if (length >= 4 && bytes[2] == 0x00.toByte() && bytes[3] == 0x00.toByte()) {
        ...
        return InputStreamReader(ByteArrayInputStream(bytes), SdkConstants.UTF_8)
    }
