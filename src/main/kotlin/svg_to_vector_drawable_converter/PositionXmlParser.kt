...
object PositionXmlParser {
    ...
    private const val NAMESPACE_PREFIX_FEATURE = "http://xml.org/sax/features/namespace-prefixes" //$NON-NLS-1$
    ...
    private val ENCODING_PATTERN = Pattern.compile("encoding=['\"](\\S*)['\"]") //$NON-NLS-1$
    ...
    @JvmOverloads
    @Throws(ParserConfigurationException::class, SAXException::class, IOException::class)
    fun parse(data: ByteArray, checkDtd: Boolean = true): Document {
    ...
    @Throws(ParserConfigurationException::class, SAXException::class, IOException::class)
    private fun parse(
        xml: String, input: InputSource, checkBom: Boolean,
        checkDtd: Boolean
    ): Document {
        var xml = xml
        return try {
            ...
            if (data[lineEnd].toInt() == 0) {
                ...
                for (i in lineEnd - 4 downTo 0) {
                    if ((data[i] == 'u'.code.toByte() || data[i] == 'U'.code.toByte())
                        ...
                        && (data[i + 3] == '-'.code.toByte() || data[i + 3] == '_'.code.toByte()) && data[i + 4] == '8'.code.toByte()
                    ) {
                        charset = SdkConstants.UTF_8
                        break
                    }
        ...
        if (node is Attr) {
            ...
            if (pos != null) {
                ...
                val pattern = Pattern.compile(
                    if (attr.prefix != null) String.format(
                        "(%1\$s\\s*=\\s*[\"'].*?[\"'])",
                        name
                ...
                return if (matcher.find(startOffset) && matcher.start(1) <= endOffset) {
                    ...
                    attributePosition.setEnd(
                        Position(line, column + matcher.end(1) - index, matcher.end(1))
                    )
                    attributePosition
                } else {
            ...
            if (pos != null) {
                ...
                while (offset <= endOffset) {
                    ...
                    if (c == '>' && !inAttribute) {
                        ...
                        // Also set end range for retrieval in getLocation
                        if (end != -1) {
                            attributePosition.setEnd(Position(line, column, offset + end))
                        } else {
                            attributePosition.setEnd(
                                Position(line, column, offset + textLength)
                            )
                        }
    ...
    private class DomBuilder(private val mXml: String) : DefaultHandler2() {
        ...
        override fun endElement(uri: String, localName: String, qName: String) {
            ...
            val pos = (element.getUserData(POS_KEY) as Position)
            pos.setEnd(currentPosition)
